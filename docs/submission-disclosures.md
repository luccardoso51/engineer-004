...
- **Directed (not merely approved) #1 — preserving governance docs.** Mid-way through the anomaly-detection ticket, Lucas stopped the flow: "before we continue, I'm horrified about this... these docs are very important, it should be in the repo," directing that `SCORING.md`, `scoring_rubric.md`, and `docs/adr/to-spec.md` not be dropped from the repo.
